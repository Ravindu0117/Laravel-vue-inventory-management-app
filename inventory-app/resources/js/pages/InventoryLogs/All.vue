...
<template>
  <div class="min-h-screen bg-purple-100 p-6 space-y-6">
    ...
    <div v-for="item in items" :key="item.id" class="bg-white shadow rounded-lg p-4">
      <h2 class="text-xl font-semibold text-gray-700">
        {{ item.name }} ({{ item.measurement_unit }})
      </h2>
      <p class="text-gray-500">Current Stock: <span class="font-bold">{{ item.quantity }}</span></p>
      ...
      <table class="min-w-full mt-2 border">
        ...
        <tbody>
          <tr v-for="log in item.logs" :key="log.id" class="hover:bg-gray-50">
            <td class="border px-4 py-2">
              <span
                :class="log.change_type === 'addition' ? 'bg-purple-100 text-green-700 px-2 py-1 rounded text-sm' : 'bg-red-100 text-red-700 px-2 py-1 rounded text-sm'">
                {{ log.change_type === 'addition' ? 'Addition' : 'Deduction' }}
              </span>
            </td>
            <td class="border px-4 py-2 text-right text-black">{{ log.quantity }}</td>
            <td class="border px-4 py-2 text-black">{{ log.description ?? '-' }}</td>
            <td class="border px-4 py-2 text-black">{{ new Date(log.created_at).toLocaleString() }}</td>
          </tr>
          <tr v-if="item.logs.length === 0">
            <td colspan="4" class="text-center py-2 text-gray-500">No history</td>
          </tr>
        </tbody>
      </table>
    </div>
